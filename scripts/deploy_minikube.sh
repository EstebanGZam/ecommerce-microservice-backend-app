...
echo "Esperando a que el namespace 'dev' esté creado..."
RETRY_COUNT=0
...
until ${NAMESPACE_READY} || [ ${RETRY_COUNT} -eq ${MAX_RETRIES} ]; do
    if kubectl get namespace dev > /dev/null 2>&1; then
        echo "Namespace 'dev' encontrado."
        NAMESPACE_READY=true
    else
        echo "Namespace 'dev' aún no encontrado, reintentando en 1 segundo..."
        sleep 1
...
if ! ${NAMESPACE_READY}; then
    echo "Error: El namespace 'dev' no se pudo crear después de ${MAX_RETRIES} segundos. Abortando."
    exit 1
fi
...
kubectl rollout status deployment/zipkin -n dev --timeout=120s
...
kubectl rollout status deployment/service-discovery -n dev --timeout=180s
...
kubectl rollout status deployment/cloud-config -n dev --timeout=120s
...
echo "Verifica el estado de los pods con: kubectl get pods -n dev"
echo "Verifica el estado de los services con: kubectl get services -n dev"
